...
#
# Driven by the skill (not a hook): the Claude Code surface runs on the host,
# so the browser opens normally and the localhost OAuth callback reaches the CLI.
#
set -euo pipefail
...
# Harmless no-op when no proxy is configured; lets the CLI's fetch use a proxy
# if one happens to be set in the environment.
